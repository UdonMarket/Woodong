...
var marker = new kakao.maps.Marker({
	...
	var locations = [		
 	...
 	for (i=0; i<locations.length; i++) { 
		...
		var markerImage = new kakao.maps.MarkerImage(imageSrc, imageSize, imageOption);
	    //markerPosition = new kakao.maps.LatLng(37.54699, 127.09598); // 마커가 표시될 위치입니다
	    marker = new kakao.maps.Marker({ // 마커를 생성하고 지도에 표시합니다
	        map: map,
	        image: markerImage, // 마커이미지 설정 
 	...
 	function asas(locations){
 		var location = locations;
	        ...
	        '  <a href="javascript:openWin('+location[1]+','+location[2]+');" >' +
...
function openWin(wlat,wlng){
	
	 window.open("https://maps.google.com/maps?z=16&q="+wlat+","+wlng,""
			 ,"height=640, width=480, top=200, left=700,status=no,scrollbars=auto,menubar=no directories=no, location=no, menubar=no, resizable=no, status=yes, toolbar=no"); 
